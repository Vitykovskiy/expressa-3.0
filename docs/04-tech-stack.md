...
Status: `Customer and backoffice frontend implementations selected on top of the validated Stage 0 substrate`

The deployment substrate from Stage 0 remains fixed. The customer-facing WebApp and the role-aware backoffice WebApp are implemented as dedicated React/Vite applications that are built into static assets and served by the existing nginx frontend container.

## Candidate Stack Summary

| Area | Selected | Why | Alternatives | Risks |
| --- | --- | --- | --- | --- |
| Frontend | `React 18 + TypeScript + Vite 6`, built in `node:24-alpine` and served by `nginx:1.27-alpine` | Matches the Telegram WebApp interaction model, keeps the runtime static and simple, and reuses the validated Stage 0 ingress/container contract for both customer and backoffice surfaces | SSR, Next.js, pure static placeholder | Until backend issues `#8` and `#11` land, both apps must tolerate missing live endpoints |
...
- React:
  ...
  - https://react.dev/
  Key best practices:
  - keep UI state localized to each app boundary;
  - drive view state from API/session contracts instead of ad hoc DOM mutations;
  - cover critical customer and backoffice flows with automated UI tests.
  Project conventions:
  - `apps/customer-web/` is the customer Telegram WebApp implementation;
  - `apps/backoffice-web/` is the role-aware backoffice WebApp implementation;
  - loading, forbidden, blocked, and empty states are first-class surfaces, not incidental fallbacks.

- Vite:
  Official docs:
  - https://vite.dev/guide/
  Key best practices:
  - keep each app as a static build artifact behind nginx;
  ...
  Project conventions:
  - the frontend image builds from both `apps/customer-web/` and `apps/backoffice-web/`;
  - customer assets are served from `/`, backoffice assets from `/backoffice/`;
  - `npm run build` inside each app is the canonical frontend artifact build.
...
- Stage 0 introduced placeholder containers to prove delivery infrastructure; the customer and backoffice frontends now replace the placeholder frontend internals without changing the deployment substrate.
- Local and CI validation use the same compose file with different env files.
- Staging deploys to `/opt/expressa-3.0` on the VPS and exposes customer `/` plus backoffice `/backoffice/` on port `8080`.
...
- Both frontend apps must stay compatible with a partially implemented backend by providing deterministic blocked/error/mock behavior for QA and staging verification.

## Risks

- The backend is still ahead-of-contract for customer APIs until issue `#8` lands and for backoffice APIs until issue `#11` lands, so both frontend apps need explicit live-to-mock fallback paths for staging and QA.
